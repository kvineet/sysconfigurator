...
public class EncryptorTableModel implements TableModel {
  ...
  private boolean checkDuplicate(String aValue, int rowIndex, String name) {
    ...
    String newVal = columns.stream().filter(e -> e.isPrimaryKey()).map(e -> {
    ...
    for (Map<String, String> data : dataSet) {
      String pkeyValue = columns.stream().filter(e -> e.isPrimaryKey())
          .map(e -> data.get(e.getName())).collect(Collectors.joining(""));

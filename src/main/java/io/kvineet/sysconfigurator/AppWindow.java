...
public class AppWindow {
    ...
    private static final List<String> keyLengths =
            Stream.of("128 Bit", "256 Bit").collect(Collectors.toList());
    ...
    private void initialize() {
        ...
        panel.setLayout(new FormLayout(
                new ColumnSpec[] {ColumnSpec.decode("46px"), ColumnSpec.decode("88px"),
                        ...
                        FormSpecs.LABEL_COMPONENT_GAP_COLSPEC, ColumnSpec.decode("170px"),},
                new RowSpec[] {FormSpecs.LABEL_COMPONENT_GAP_ROWSPEC, RowSpec.decode("25px"),
                        FormSpecs.LINE_GAP_ROWSPEC, RowSpec.decode("19px"),
                        FormSpecs.LINE_GAP_ROWSPEC, RowSpec.decode("19px"),
                        FormSpecs.LINE_GAP_ROWSPEC, RowSpec.decode("19px"),
                        FormSpecs.LINE_GAP_ROWSPEC, RowSpec.decode("25px"), RowSpec.decode("35px"),
                        RowSpec.decode("25px"), FormSpecs.LINE_GAP_ROWSPEC,
                        RowSpec.decode("25px"),}));
        ...
        btnNewButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent arg0) {
                ...
                String input = (String) JOptionPane.showInputDialog(null,
                        "Choose the key length (256 Bit recomended.)", "AES Key Length",
                        JOptionPane.QUESTION_MESSAGE, null, keyLengths.toArray(),
                        keyLengths.get(DEFAULT_SELECT)); // Initial choice
                int index = keyLengths.indexOf(input);
                String newKey = null;
                if (index > -1) {
                    try {
                        newKey = EncryptionUtil.generateKey(keyBits.get(index));
